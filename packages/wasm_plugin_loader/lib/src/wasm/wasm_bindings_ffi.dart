...
typedef WasmFuncCallbackC = ffi.Pointer<wasm_trap_t> Function(
...
class WasmerBindings {
  WasmerBindings(ffi.DynamicLibrary lib) : _b = NativeWasmerBindings(lib);
  final NativeWasmerBindings _b;

  // Engine / Store
  ffi.Pointer<WasmEngineT> engineNew() => _b.wasm_engine_new();
  ffi.Pointer<WasmStoreT> storeNew(ffi.Pointer<WasmEngineT> e) =>
      _b.wasm_store_new(e);
  ...
  void byteVecNewEmpty(ffi.Pointer<WasmByteVec> out) =>
      _b.wasm_byte_vec_new_empty(out);
  void byteVecDelete(ffi.Pointer<WasmByteVec> v) => _b.wasm_byte_vec_delete(v);

  // Module
  ffi.Pointer<WasmModuleT> moduleNew(
          ffi.Pointer<WasmStoreT> s, ffi.Pointer<WasmByteVec> bytes) =>
  ...
  void moduleImports(
      ...
      _b.wasm_module_imports(m, out);
  void moduleExports(
          ffi.Pointer<WasmModuleT> m, ffi.Pointer<WasmExporttypeVec> out) =>
      _b.wasm_module_exports(m, out);

  // ImportType / ExportType
  ffi.Pointer<WasmByteVec> importtypeModule(ffi.Pointer<WasmImporttypeT> it) =>
      _b.wasm_importtype_module(it);
  ffi.Pointer<WasmByteVec> importtypeName(ffi.Pointer<WasmImporttypeT> it) =>
      _b.wasm_importtype_name(it);
  ffi.Pointer<WasmExterntypeT> importtypeType(
          ffi.Pointer<WasmImporttypeT> it) =>
      _b.wasm_importtype_type(it);
  void importtypeVecNewEmpty(ffi.Pointer<WasmImporttypeVec> out) =>
      _b.wasm_importtype_vec_new_empty(out);
  void importtypeVecDelete(ffi.Pointer<WasmImporttypeVec> v) =>
      _b.wasm_importtype_vec_delete(v);
  ffi.Pointer<WasmByteVec> exporttypeName(ffi.Pointer<WasmExporttypeT> et) =>
      _b.wasm_exporttype_name(et);
  void exporttypeVecNewEmpty(ffi.Pointer<WasmExporttypeVec> out) =>
      _b.wasm_exporttype_vec_new_empty(out);
  void exporttypeVecDelete(ffi.Pointer<WasmExporttypeVec> v) =>
      _b.wasm_exporttype_vec_delete(v);

  // ExternType
  ffi.Pointer<WasmFunctypeT> externtypeAsFunctypeConst(
          ffi.Pointer<WasmExterntypeT> et) =>
  ...
  void externVecDelete(ffi.Pointer<WasmExternVec> v) =>
      _b.wasm_extern_vec_delete(v);

  // Extern
  ffi.Pointer<WasmFuncT> externAsFunc(ffi.Pointer<WasmExternT> e) =>
      _b.wasm_extern_as_func(e);
  ffi.Pointer<WasmMemoryT> externAsMemory(ffi.Pointer<WasmExternT> e) =>
      _b.wasm_extern_as_memory(e);
  ...
  void instanceDelete(ffi.Pointer<WasmInstanceT> i) =>
      _b.wasm_instance_delete(i);
  ...
  ffi.Pointer<ffi.Uint8> memoryData(ffi.Pointer<WasmMemoryT> m) =>
      _b.wasm_memory_data(m).cast<ffi.Uint8>();
  int memoryDataSize(ffi.Pointer<WasmMemoryT> m) =>
      _b.wasm_memory_data_size(m);

  // Trap
  void trapMessage(
          ffi.Pointer<WasmTrapT> t, ffi.Pointer<WasmByteVec> out) =>
      _b.wasm_trap_message(t, out);
  ...
  static String readByteVec(ffi.Pointer<WasmByteVec> vec) {
    final bytes = List<int>.generate(
        vec.ref.size, (i) => (vec.ref.data + i).value & 0xFF);
