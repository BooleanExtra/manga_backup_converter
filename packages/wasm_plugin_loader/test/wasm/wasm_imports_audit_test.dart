...
const _registeredImports = <String>{
  // std module
  'std::destroy',
  'std::buffer_len',
  'std::_read_buffer',
...
const _knownStubModules = {'canvas', 'js'};
...
void _printTable(
  ...
  for (final imp in imports) {
    ...
    final isStubModule = _knownStubModules.contains(imp.module);
...
const _fixtures = <({String label, String wasmPath})>[
...
void main() {
  final missingFixtures = _fixtures.where((f) => !_findFixture(f.wasmPath).existsSync()).map((f) => f.label).toList();

  final skipReason = !_hasWasmer()
    ...
    () {
      ...
      setUpAll(() {
        for (final f in _fixtures) {
          final file = _findFixture(f.wasmPath);
      ...
      for (final fixture in _fixtures) {
        test('${fixture.label} — all imports registered or known-stub', () {
          final imports = allImports[fixture.label];
          if (imports == null) {
          ...
          final unexpected = imports.where((imp) {
            final key = '${imp.module}::${imp.name}';
      ...
      test('aggregate — print unique imports across all fixtures', () {
        ...
        for (final f in _fixtures) {
          for (final imp in allImports[f.label] ?? <({String module, String name, int resultKind})>[]) {
            final key = '${imp.module}::${imp.name}';
            final entry = unique[key];
            if (entry == null) {
              unique[key] = (resultKind: imp.resultKind, fixtures: [f.label]);
            } else {
        ...
        final sorted = unique.entries.toList()..sort((a, b) => a.key.compareTo(b.key));
        ...
        for (final e in sorted) {
          ...
          final mod = parts[0];
          final nm = parts[1];
          final fixtures = e.value.fixtures;
