...
void main() {
  group('determineStrategy', () {
    test('same non-Tachi format returns Migration', () {
      ...
      check(determineStrategy(const Paperback(), const Paperback())).isA<Migration>();
      check(determineStrategy(const Tachimanga(), const Tachimanga())).isA<Migration>();
      check(determineStrategy(const Mangayomi(), const Mangayomi())).isA<Migration>();
    });

    test('same or cross Tachi fork returns Migration', () {
      check(determineStrategy(const Mihon(), const Mihon())).isA<Migration>();
      check(determineStrategy(const TachiSy(), const TachiSy())).isA<Migration>();
      check(determineStrategy(const Mihon(), const TachiSy())).isA<Migration>();
      check(determineStrategy(const TachiJ2k(), const TachiYokai())).isA<Migration>();
      check(determineStrategy(const TachiNeko(), const Mihon())).isA<Migration>();
      check(determineStrategy(const TachiSy(), const TachiJ2k())).isA<Migration>();
    });

    test('Tachi fork to Tachimanga returns DirectConversion', () {
      check(determineStrategy(const Mihon(), const Tachimanga())).isA<DirectConversion>();
      check(determineStrategy(const TachiSy(), const Tachimanga())).isA<DirectConversion>();
      check(determineStrategy(const TachiJ2k(), const Tachimanga())).isA<DirectConversion>();
    });
