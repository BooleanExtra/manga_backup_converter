...
void main() {
  group('Wasm3Runner', () {
    test('add(3, 4) returns 7', () async {
      final Wasm3Runner runner = await Wasm3Runner.fromBytes(_addModule);
      try {
    ...
    test('memory read/write via WASM functions', () async {
      final Wasm3Runner runner = await Wasm3Runner.fromBytes(_memoryModule);
      try {
    ...
    test('host import callback', () async {
      var callCount = 0;
      final Wasm3Runner runner = await Wasm3Runner.fromBytes(
        _importModule,
        imports: {
          'env': {
            'get_value': () {
    ...
    test('unregistered import returns -1 stub', () async {
      ...
      final Wasm3Runner runner = await Wasm3Runner.fromBytes(_importModule);
      try {
        // -1 + 10 = 9
        final Object? result = runner.call('call_import', []);
    ...
    test('invalid WASM binary throws', () async {
      await check(
        Wasm3Runner.fromBytes(Uint8List.fromList([0, 1, 2, 3])),
      ).throws<WasmRuntimeException>();
    });
  });
}
