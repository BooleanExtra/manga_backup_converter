...
- **Wasm3 LEB fix**: vendored wasm3 `m3_compile.c` patched — `ReadLEB_i7` → `ReadLEB_u32` for table/memory index operands (`call_indirect`, `memory.size`, `memory.grow`, `memory.copy`, `memory.fill`); original wasm3 used 7-bit LEB limit but WASM spec uses `u32` and Rust toolchains emit padded multi-byte encodings
...
- **Windows DLL loading**: `dartjni.dll` depends on `jvm.dll` at load time; `JreManager._addDllDirectory` prepends the JVM server dir to process PATH via Win32 `SetEnvironmentVariableW` before `Jni.spawnIfNotExists`
