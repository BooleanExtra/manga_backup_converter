...
- **Postcard zigzag web safety**: JS bitwise operators (`^`, `>>`, `<<`) work on 32-bit signed ints — zigzag encode/decode must use arithmetic (`isOdd`, `~/ 2`, `* 2`) not bitwise ops; see `postcard_reader.dart` and `postcard_writer.dart`
...
- **SwiftSoup cross-compilation**: macOS builds use `--triple arm64-apple-macosx10.15` / `x86_64-apple-macosx10.15` (not bare `swift build -c release`); iOS builds need `-Xswiftc -sdk -Xswiftc <path>` from `xcrun --sdk iphoneos --show-sdk-path`; build hook uses `swift build --show-bin-path` to locate the output directory (SPM may place it differently across versions)
