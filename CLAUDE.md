...
- `HostStore` is shared between web (Worker) and native (WASM isolate) — it stores raw `Uint8List` resources; decoding postcard results must happen at the consumer level (`aidoku_plugin_io.dart` / `aidoku_plugin_web.dart`), not inside `HostStore`, because the native isolate forwards raw bytes via `WasmPartialResultMsg`
...
- **Aidoku web WASM**: Web runs WASM in a JS Web Worker (`lib/src/web/wasm_worker_js.dart`) with sync XHR for HTTP; sync XHR is used because WASM host imports are synchronous (they must return a value immediately) and sync XHR is allowed in workers (only deprecated on main thread), avoiding SharedArrayBuffer/COOP+COEP requirements; main thread communicates via `postMessage`; the JS source is embedded as a `const String` and loaded as a Blob URL
